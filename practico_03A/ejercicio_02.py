...
def agregar_persona(nombre, nacimiento, dni, altura,session, id =0):
    try:
        oper = Persona()
        if id !=0:
            oper.id_persona = id
...
def pruebas(session):
    ...
    id_marcelina = agregar_persona('marcela gonzalez', datetime.datetime(1980, 1, 25), 12164492, 195,session, 1)
